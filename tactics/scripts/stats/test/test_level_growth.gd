...
func _exit_tree():
	for actor in heroes:
		...
		stats.did_change_notification(StatTypes.Stat.LVL).disconnect(OnLevelChange)	
		stats.will_change_notification(StatTypes.Stat.EXP).disconnect(OnExperienceException)

func VerifyLevelToExperienceCalculations():
	for i in range(1,100):
		var expLvl: int = Rank.experience_for_level(i)
		var lvlExp: int = Rank.level_for_experience(expLvl)
...
func VerifySharedExperienceDistribution():
	...
	for hero in party:
		...
		stats.did_change_notification(StatTypes.Stat.LVL).connect(OnLevelChange)	
		stats.will_change_notification(StatTypes.Stat.EXP).connect(OnExperienceException)
...
func OnExperienceException(sender:Stats, vce:ValueChangeException):
	...
	match roll:
		...
		1:
			vce.add_modifier(AddValueModifier.new(0,1000))
			print("{0} would have received {1} experience, but we added 1000".format([actor.name, vce.delta]))
		2:
			vce.add_modifier(MultDeltaModifier.new(0,2))
