...
func _ready():
	...
	anchorCenter.set_values("Center",Control.PRESET_CENTER, Control.PRESET_CENTER, Vector2.ZERO, .5, Tween.TRANS_BACK )
	anchorList.append(anchorCenter)


func GetAnchor(anchorName: String):
	for anchor in self.anchorList:
		if anchor.anchor_name == anchorName:
			return anchor
	return null
...
func AnchorButton(text:String):
	...
	if anchor:
		childPanel.to_anchor_position(anchor, animated)
	else:
